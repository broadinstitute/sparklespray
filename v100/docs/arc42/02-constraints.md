...
- Coexists in the same repository as the legacy Python `sparkles`
  implementation and the separate Node/npm `dashboard` frontend and
  `dashboard-backend` Python service; v100 reimplements its own
  dashboard-backend in Go (`v100/dev/dashboard_backend.go`) rather than
  reusing the Python one, while targeting the same `openapi.yaml` contract.
