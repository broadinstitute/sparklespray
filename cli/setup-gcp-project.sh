...
# sprinkles depends on, creates a single service account, grants it every
...
# `sprinkles dev bootstrap-project`, which does the rest: Firestore database +
# indexes, GCS bucket, Pub/Sub topics, SprinklesConfig, and an initial API key).
#
# Usage: setup-gcp-project.sh --project <id> [--service-account-name <name>] [--key-file <path>]
set -euo pipefail

SERVICE_ACCOUNT_NAME="sprinkles"
KEY_FILE="./sprinkles-sa-key.json"
PROJECT=""
...
# of firewall rules, and sprinkles's jobs are single-VM with no
...
# Pub/Sub tokens by impersonating SprinklesConfig.subscriber_sa. In this
...
Done. Next steps:
  ...
  sprinkles dev bootstrap-project \\
    --project ${PROJECT} \\
    --region <gcp-region> \\
    --bucket <bucket-name> \\
...
This key file grants broad access to sprinkles's own resources in this
