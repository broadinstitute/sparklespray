...
func TestLocalizeAndCommand(t *testing.T) {
	...
	if _, err := fsClient.Collection(sprinkles.JobCollection).Doc(jobID).Set(ctx, job); err != nil {
	...
	task := sprinkles.Task{
		...
		FilesToLocalize: []sprinkles.FileToLocalize{
	...
	if _, err := fsClient.Collection(sprinkles.TaskCollection).Doc(taskID).Set(ctx, task); err != nil {
		t.Fatalf("writing task: %v", err)
	}

	statuses := waitForAllTasksTerminal(t, ctx, fsClient, jobID, 60*time.Second)
	if statuses[taskID] != sprinkles.StatusSuccess {
		t.Fatalf("task %s: want status=%s, got %s", taskID, sprinkles.StatusSuccess, statuses[taskID])
	}
...
func TestSubmitAndComplete(t *testing.T) {
	...
	for taskID, status := range statuses {
		if status != sprinkles.StatusSuccess {
			t.Errorf("task %s: want status=%s, got %s", taskID, sprinkles.StatusSuccess, status)
		}
	}
}
...
func TestKillJob(t *testing.T) {
	...
	waitForAnyTaskInStates(t, ctx, fsClient, jobID,
		[]string{sprinkles.StatusClaimed, sprinkles.StatusRunning}, 30*time.Second)
	...
	for _, status := range taskStatuses(t, ctx, fsClient, jobID) {
		if status == sprinkles.StatusPending {
			pendingCount++
		}
	}
	if pendingCount == 0 {
	...
	if err := sprinkles.KillJob(ctx, testProject, testDB, jobID); err != nil {
	...
	for taskID, status := range statuses {
		if status != sprinkles.StatusKilled {
			t.Errorf("task %s: want status=%s, got %s", taskID, sprinkles.StatusKilled, status)
		}
	}
}
...
// sprinkles.KillJob directly, to verify the HTTP route itself (registration,
...
func TestCancelJobViaAPI(t *testing.T) {
	...
	// NewDashboardHandler requires a SprinklesConfig/default doc to exist; an
	// all-zero config is fine since cancelling a job doesn't touch any of
	// its fields.
	if _, err := fsClient.Collection("SprinklesConfig").Doc("default").Set(ctx, dev.SprinklesConfig{}); err != nil {
		t.Fatalf("writing SprinklesConfig/default: %v", err)
	}
...
// Firestore/PubSub clients, seeding the SprinklesConfig doc and an API key it
...
func TestUpdateWorkpoolViaAPI(t *testing.T) {
	...
	writeWorkPool(t, ctx, fsClient, &sprinkles.WorkPool{
		WorkpoolID:                   workpoolID,
		MaxWorkerCount:               10,
	...
	snap, err := fsClient.Collection(sprinkles.WorkpoolCollection).Doc(workpoolID).Get(ctx)
	if err != nil {
