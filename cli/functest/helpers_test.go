...
// writeWorkPool writes a sprinkles.WorkPool doc directly to Firestore.
func writeWorkPool(t *testing.T, ctx context.Context, fs *firestore.Client, wp *sprinkles.WorkPool) {
	t.Helper()
	if _, err := fs.Collection(sprinkles.WorkpoolCollection).Doc(wp.WorkpoolID).Set(ctx, wp); err != nil {
...
func ensureTopics(t *testing.T, ctx context.Context, psClient *pubsub.Client) {
	t.Helper()
	for _, name := range []string{"sprinkles-events", "sprinkles-worker-in"} {
...
func startWorker(t *testing.T, ctx context.Context, workpoolID string) {
	...
	go func() {
		err := sprinkles.RunWorker(ctx, sprinkles.WorkerRunConfig{
			Project:    testProject,
			DB:         testDB,
			WorkerID:   workerID,
...
func waitForAllTasksTerminal(t *testing.T, ctx context.Context, fsClient *firestore.Client, jobID string, timeout time.Duration) map[string]string {
	...
	terminalStatuses := map[string]bool{
		sprinkles.StatusSuccess: true,
		sprinkles.StatusError:   true,
		sprinkles.StatusFailed:  true,
		sprinkles.StatusKilled:  true,
	}
	deadline := time.Now().Add(timeout)
	for {
		if time.Now().After(deadline) {
...
func taskStatuses(t *testing.T, ctx context.Context, fsClient *firestore.Client, jobID string) map[string]string {
	t.Helper()
	iter := fsClient.Collection(sprinkles.TaskCollection).
		Where("job_id", "==", jobID).
		Documents(ctx)
	defer iter.Stop()
