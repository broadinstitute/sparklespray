...
import (
	...
	"github.com/broadinstitute/sprinkles/scheduler"
...
func newMonitor(
	...
	topicName := fmt.Sprintf("projects/%s/topics/sprinkles-events", project)
	if _, err := psClient.TopicAdminClient.CreateTopic(ctx, &pubsubpb.Topic{Name: topicName}); err != nil {
		if grpcstatus.Code(err) != codes.AlreadyExists {
			return nil, nil, fmt.Errorf("ensuring sprinkles-events topic: %w", err)
		}
	}
	...
	resultWriter := sprinkles.NewJobResultWriter(sprinkles.NewFirestoreTaskQueue(fsClient, nil), gcsClient)
