...
func runDevTestProfileCommand(c *cli.Context) error {
	...
	args := []string(c.Args())
	if len(args) < 2 {
		return fmt.Errorf("usage: sprinkles dev test-profile-command [--interval DURATION] [--container NAME] [--docker-arg ARG]... [--] <docker-image> <command...>")
	}
	image := args[0]
	command := args[1:]

	workDir, err := os.MkdirTemp("", "sprinkles-test-profile-*")
	if err != nil {
		return fmt.Errorf("creating work dir: %w", err)
	}
	defer os.RemoveAll(workDir)

	containerName := "sprinkles-test-profile-" + uuid.New().String()[:8]
	...
	fmt.Fprintf(os.Stderr, "Running: %s %s\n", sprinkles.DockerExecutable, strings.Join(dockerArgs, " "))
	describeSchedule(c, os.Stderr)

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	cmd := exec.CommandContext(ctx, sprinkles.DockerExecutable, dockerArgs...)
	...
	sampler := sprinkles.NewMetricSampler(containerName, workDir, containerName)
	...
	if rmOut, rmErr := exec.Command(sprinkles.DockerExecutable, "rm", "-f", containerName).CombinedOutput(); rmErr != nil {
...
func profileExistingContainer(c *cli.Context, name string) error {
	...
	sampler := sprinkles.NewMetricSampler(name, workDir, name)
	enc := json.NewEncoder(os.Stdout)
	enc.SetIndent("", "  ")
...
func firstDelay(c *cli.Context) time.Duration {
	...
	return sprinkles.MetricsFirstDelay
}
...
func nextDelay(c *cli.Context, cur time.Duration) time.Duration {
	...
	return sprinkles.NextMetricsDelay(cur)
}
...
func describeSchedule(c *cli.Context, w *os.File) {
	...
	fmt.Fprintf(w, "Sampling metrics on the adaptive schedule (%s, doubling to %s)\n",
		sprinkles.MetricsFirstDelay, sprinkles.MetricsMaxDelay)
}
