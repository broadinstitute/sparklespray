		...
		Subcommands: []cli.Command{
			{
				...
				Usage:     "Load a SprinklesConfig JSON file into Firestore (SprinklesConfig/default), used by dashboard-backend",
				Flags: []cli.Flag{
			...
			},
			{
				...
				Usage: "Start only the HTTP dashboard backend (see \"sprinkles serve\" to run it together with the monitor)",
				Flags: []cli.Flag{
					cli.StringFlag{Name: "project"},
					cli.StringFlag{Name: "db", Value: defaultDB},
					cli.StringFlag{Name: "addr", Value: ":8080", Usage: "address to listen on"},
					cli.StringFlag{Name: "prefix", Usage: "URL path prefix under which all routes are served, e.g. \"sprinkles\" serves everything under /sprinkles/... (default: none, serve at the root)"},
				},
				Action: runDevDashboardBackend,
			},
			{
				Name:  "monitor",
				Usage: "Start only the monitor (see \"sprinkles serve\" to run it together with the dashboard backend)",
				Flags: []cli.Flag{
			...
			{
				Name:  "create-topics",
				Usage: "Create all Pub/Sub topics and subscriptions required by sprinkles",
				Flags: []cli.Flag{
					cli.StringFlag{Name: "project"},
				},
			...
			},
			{
				...
				Usage: "Set up everything sprinkles needs in a GCP project that already has APIs enabled and a service account with the required IAM roles (see v100/setup-gcp-project.sh): Firestore database + composite indexes, GCS bucket, Pub/Sub topics, SprinklesConfig, and an initial API key",
				Flags: []cli.Flag{
					cli.StringFlag{Name: "project"},
					cli.StringFlag{Name: "db", Value: defaultDB},
