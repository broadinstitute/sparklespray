...
func runDevSimulate(c *cli.Context) error {
	...
	for _, topicShortName := range []string{"sprinkles-events", "sprinkles-worker-in"} {
	...
	ep := sprinkles.NewEventPublisher(psClient.Publisher("sprinkles-events"), fsClient)
	defer ep.Stop()

	queue := sprinkles.NewFirestoreTaskQueue(fsClient, ep)
	...
	if _, err := fsClient.Collection(sprinkles.WorkpoolCollection).Doc(cfg.workpoolID).Set(ctx, workpool); err != nil {
...
func runSimJobSubmitter(ctx context.Context, cfg simConfig, fsClient *firestore.Client, ep *sprinkles.EventPublisher, jobSummaries *monitor.FirestoreJobSummaryStore) {
	jobCounter := 0
	first := true
	for ctx.Err() == nil {
		...
		job := sprinkles.Job{
			...
			Resources:  []sprinkles.ResourceEntry{{Name: "slots", Value: 1}},
		}

		err := fsClient.RunTransaction(ctx, func(ctx context.Context, tx *firestore.Transaction) error {
			if err := tx.Set(fsClient.Collection(sprinkles.JobCollection).Doc(jobID), job); err != nil {
				return err
			}
			for i, taskID := range taskIDs {
				...
				if err := tx.Set(fsClient.Collection(sprinkles.TaskCollection).Doc(taskID), task); err != nil {
					return err
				}
			}
			return nil
		...
		if err := ep.PublishJobCreated(ctx, sprinkles.JobCreatedEvent{
			Type:       "job_created",
			JobID:      jobID,
			WorkpoolID: cfg.workpoolID,
		}); err != nil {
...
func runSimWorker(ctx context.Context, cfg simConfig, batchID string, fsClient *firestore.Client, ep *sprinkles.EventPublisher, queue sprinkles.TaskQueue) {
	...
	for ctx.Err() == nil {
		...
		if _, err := workerDoc.Set(ctx, sprinkles.WorkerRecord{
			WorkerID:        workerID,
			WorkpoolID:      cfg.workpoolID,
			BatchID:         batchID,
		...
		if err := ep.PublishWorkerEvent(ctx, sprinkles.WorkerEvent{
...
func simWorkerLifetime(ctx context.Context, preemptCtx context.Context, cfg simConfig, workerID string, queue sprinkles.TaskQueue) {
	for preemptCtx.Err() == nil {
		...
		currentState := sprinkles.StatusClaimed
		taskStart := time.Now()
		...
		if err := queue.UpdateState(preemptCtx, claimed.TaskID, sprinkles.StatusClaimed, sprinkles.StatusRunning); err != nil {
			log.Printf("simulate: claimed→running: %v", err)
			continue
		}
		currentState = sprinkles.StatusRunning
		...
		if err := queue.UpdateState(preemptCtx, claimed.TaskID, sprinkles.StatusRunning, sprinkles.StatusWriting); err != nil {
			log.Printf("simulate: running→writing: %v", err)
			continue
		}
		currentState = sprinkles.StatusWriting
		...
		if exitCode != 0 {
			...
			if err := queue.UpdateState(preemptCtx, claimed.TaskID, sprinkles.StatusWriting, sprinkles.StatusSuccess); err != nil {
				log.Printf("simulate: writing→success: %v", err)
			}
		}

		if err := queue.RecordResourceUsage(preemptCtx, claimed.TaskID, &sprinkles.ResourceUsage{
			StartTime:      taskStart,
			EndTime:        time.Now(),
...
func runSimJobSummaryUpdater(ctx context.Context, workpoolID string, jobSummaries *monitor.FirestoreJobSummaryStore, taskStore *monitor.FirestoreTaskStore, ep *sprinkles.EventPublisher) {
...
func runSimWorkPoolSummaryUpdater(ctx context.Context, workpoolID string, fsClient *firestore.Client, batchStore *monitor.FirestoreBatchRequestStore, taskStore *monitor.FirestoreTaskStore, workerStore *monitor.FirestoreWorkerStore, workPoolSummaryStore *monitor.FirestoreWorkPoolSummaryStore) {
	...
	for {
		...
		case <-ticker.C:
			...
			_, err = fsClient.Collection(sprinkles.WorkpoolCollection).Doc(workpoolID).Update(ctx, []firestore.Update{
