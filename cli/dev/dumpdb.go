...
func runDevDumpDB(c *cli.Context) error {
	...
	fmt.Println("=== Tasks ===")
	taskDocs, err := fsClient.Collection(sprinkles.TaskCollection).Documents(ctx).GetAll()
	if err != nil {
		return fmt.Errorf("listing tasks: %w", err)
	}
	for _, doc := range taskDocs {
		var t sprinkles.Task
	...
	poolDocs, err := fsClient.Collection(sprinkles.WorkpoolCollection).Documents(ctx).GetAll()
	if err != nil {
		return fmt.Errorf("listing workpools: %w", err)
	}
	for _, doc := range poolDocs {
		var p sprinkles.WorkPool
