...
const sprinklesEventsTopic = "sprinkles-events"
...
// sprinklesEventMessage is the minimal JSON shape shared by all sprinkles-events payloads.
type sprinklesEventMessage struct {
	Type       string `json:"type"`
	JobID      string `json:"job_id"`
	WorkpoolID string `json:"workpool_id"`
}

// GCPJobEventReceiver implements JobEventReceiver by pulling from the sprinkles-events
...
func NewGCPJobEventReceiver(ctx context.Context, project string) (*GCPJobEventReceiver, error) {
	...
	if err := ensurePubSubResources(ctx, client, project, sprinklesEventsTopic, monitorEventsSubscription); err != nil {
		client.Close()
