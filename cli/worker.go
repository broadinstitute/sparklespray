...
const workerOutTopic = "sprinkles-events"
const workerInTopic = "sprinkles-worker-in"
...
// "sprinkles dev test-profile-command") exec the same docker binary
...
func executeDockerCommand(ctx context.Context, imageName string, command []string, workDir string, extraDockerArgs []string, tel *TaskEventLog) (*ResourceUsage, error) {
	...
	containerName := "sprinkles-" + uuid.New().String()[:8]
