#!/usr/bin/env bash
# Builds the "sprinkles" binary: worker + submit/kill CLI + serve (monitor +
...
# via "sprinkles serve" -- see docs/deploying-dashboard-backend.md.
#
...
EMBED_DIST="${REPO_ROOT}/cli/dev/webui/dist"
...
BUILD_MARKER="${REPO_ROOT}/cli/dev/webui/.frontend-build-marker"
...
OUTPUT="${REPO_ROOT}/cli/bin/sprinkles-linux-amd64-${VERSION}"
...
if [ "${skip_frontend_build}" -eq 1 ]; then
  ...
  echo "  git checkout -- cli/dev/webui/dist/index.html"
  echo
fi
...
( cd "${REPO_ROOT}/cli" && \
  CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build \
    -ldflags "-X github.com/broadinstitute/sprinkles/dev.Version=${VERSION}" \
    -o "${OUTPUT}" \
    ./cmd/sprinkles )
