...
def cluster_store(datastore_client):
    from sparklespray.cluster_store import ClusterStore, ClusterConfig

    cluster_store = MagicMock(spec=ClusterStore)
    # Mock create_cluster to return a ClusterConfig without actually creating topics
    cluster_store.create_cluster.return_value = ClusterConfig(
        cluster_id="test-cluster",
        incoming_topic="sparkles-inbound",
        response_topic="sparkles-outbound",
    )
    return cluster_store


@pytest.fixture
def job_queue(datastore_client, task_storage, cluster_store):
...
"""
    ...
    config = load_config(
        str(config_file), verbose=False, gcloud_config_file=None, overrides={}
    )
...
def test_submit_cmd_basic(
    ...
    args = parse_args_for_test(
        ["sub", "--name", "test-job", "--push", temp_file, "echo", "hello", "world"]
    )
    ...
    args = parse_args_for_test(
        ["sub", "--name", "test-job", "--skipifexists", "echo", "hello", "world"]
    )
    result = submit_cmd(job_queue, mock_io, datastore_client, cluster_api, args, config)
...
@patch("sparklespray.commands.submit.watch")
def test_submit_cmd_with_seq_parameter(
    mock_watch, job_queue, mock_io, datastore_client, cluster_api, config, task_storage
):
    # Setup mocks
    mock_watch.return_value = True
    ...
    args = parse_args_for_test(
        ["sub", "--name", "test-job", "--seq", "3", "echo", "hello", "world"]
    )
...
@patch("sparklespray.commands.submit.watch")
def test_submit_cmd_complex_args(
    mock_watch,
    job_queue,
    mock_io,
    datastore_client,
