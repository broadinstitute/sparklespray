...
// NewApp creates and configures the sprinkles CLI app with the end-user
// commands (worker, submit, kill). The "serve" and "dev" commands are
// registered by cmd/sprinkles/main.go, since they live in cli/dev and this
...
func NewApp() *cli.App {
	...
	app.Commands = []cli.Command{
		...
		},
		{
			Name:      "submit",
			ArgsUsage: "<job-json>",
			Usage:     "Submit a job to a running dashboard-backend (POST /api/v1/job). Requires the SPRINKLES_API_KEY environment variable.",
			Flags: []cli.Flag{
...
// Main runs the complete sprinkles CLI. Provided for backward compatibility;
