...
class HasAddNodes(Protocol):
    def add_nodes(self, count: int, max_retry: int, preemptable: bool):
        ...
    ...
    def poll(self, state: ClusterStateQuery):
        ...
        if additional_nodes > 0:
            ...
            if additional_non_preemptable_nodes > 0:
                assert self.non_preemptable_created <= max_non_preemptable_attempts, f"Sparkles would like to start {additional_preemptable_nodes} additional nodes, but we've already created {self.non_preemptable_created} non-preemptable nodes (max attempts allowed currently {max_non_preemptable_attempts}). This suggests we might be recreating nodes infinitely. Aborting"
