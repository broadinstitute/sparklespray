...
class StablizedSet:
    ...
    def update(self, values):
        ...
        for value in values:
            ...
            if existing is None:
                self.per_value[value] = StablizedSetElement(
                    update_count=1, first_update=now
                )
            else:
        ...
        self.latest_values = [
            value
            for value, stats in self.per_value.items()
            if stats.update_count >= self.min_update_count
            and now - stats.first_update >= self.min_time_elapsed
        ]
    ...
    def poll(self, state: ClusterStateQuery):
        ...
        if True:  # running_nodes < len( claimed_tasks ):
            ...
            if len(orphan_summary.orphaned) > 0:
                print(
                    f"Identified {len(orphan_summary.orphaned)} likely orphaned tasks"
                )
