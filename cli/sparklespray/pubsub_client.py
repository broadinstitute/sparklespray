...
# Callback receives the full message data dict
...
class PubSubMonitorClient:
    ...
    def __init__(
        ...
        # Listeners keyed by message type, values are lists of callbacks
    ...
    def _setup_subscription(self):
        ...
        def callback(message):
            try:
                ...
                # Invoke listeners for this message type
                if message_type:
                    with self._lock:
                        listeners = self._listeners.get(message_type, []).copy()
                    for listener in listeners:
                        try:
                            listener(data)
                        except Exception as listener_error:
                            log.warning(
                                f"Error in listener for {message_type}: {listener_error}"
                            )
    ...
    def add_listener(self, message_type: str, callback: MessageListener) -> None:
        """Register a callback to be invoked when a message of the given type arrives.

        The callback will be called with the full message data dict.
        Multiple listeners can be registered for the same message type.

        Args:
            message_type: The message type to listen for (e.g., "task_started",
                "task_completed", "worker_started", "worker_stopping")
            callback: Function to call when a message of this type arrives.
                Receives the full message dict as its argument.
        """
        with self._lock:
    ...
    def remove_listener(self, message_type: str, callback: MessageListener) -> bool:
        """Remove a previously registered listener.

        Args:
            message_type: The message type the listener was registered for.
