...
class ResizeCluster:
    ...
    def __call__(self, state: ClusterState, cluster_mod: ClusterMod) -> None:
        ...
        target_node_count = min(self.target_node_count,
        ...
        if target_node_count > requested_nodes:
            ...
            remaining_preempt_attempts = self.max_preemptable_attempts - \
                state.get_preempt_attempt_count()
            nodes_to_add = target_node_count - requested_nodes
            if nodes_to_add > 0:
                log.info("Currently targeting having {} nodes running, but we've only requested {} nodes. Adding {}".format(
