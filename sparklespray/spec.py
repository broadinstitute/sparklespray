...
DownloadsAndCommand = collections.namedtuple(
    "DownloadsAndCommand", "downloads command")
...
def rewrite_argvs_files_to_upload(list_of_argvs, cas_url, hash_function, is_executable_function, extra_files):
    ...
    for argv in list_of_argvs:
        ...
        l.append(DownloadsAndCommand(files_to_dl, " ".join(
            [rewrite_filenames(x) for x in argv])))
...
def make_spec_from_command(argv,
    ...
    for task_i, dl_and_command in enumerate(list_of_dl_and_commands):
        tasks.append(dict(
            ...
            uploads=dict(include_patterns=src_wildcards, exclude_patterns=[
