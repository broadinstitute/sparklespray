...
def test_expand_tasks():
    spec = {'tasks':
            [{
                ...
                'uploads': {'include_patterns': ['**'], 'exclude_patterns': [], 'dst_url': 's3://testjob/1'},
                'parameters': {}
            }, {
                'downloads': [{'dst': 'mandelbrot.py', 'src_url': 'gs://source1', 'executable': True}],
                'command': 'python3 mandelbrot.py',
                'uploads': {'include_patterns': ['**'], 'exclude_patterns': [], 'dst_url': 's3://testjob/2'},
                'parameters': {}
            }],
    ...
    expanded_spec = expand_tasks(
        spec, io, default_url_prefix, default_job_url_prefix)
    expected_tasks = [
        {
            'downloads': [{'dst': 'mandelbrot.py', 'src_url': 'gs://source1', 'executable': True}],
            'uploads': {'include_patterns': ['**'], 'exclude_patterns': [], 'dst_url': 's3://testjob/1'},
