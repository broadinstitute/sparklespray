...
- **Language/runtime**: Go (single static binary, `CGO_ENABLED=0`), built via
  `cli/build.sh`. Chosen so worker VMs need no language runtime installed.
...
- **Task execution**: user task commands run inside Docker containers on
  the worker VM (`/usr/bin/docker`, invoked by the `sprinkles worker`
...
- **No custom VM images / no Dockerfiles**: worker VMs use a stock GCP
  Batch-provided image; the `sprinkles` binary is fetched from GCS at
  startup rather than baked into an image.
- **Module path**: `github.com/broadinstitute/sprinkles` (`go.mod`).

## 2.2 Organizational Constraints

- Developed and operated within CDS for internal ad-hoc batch
  workloads.
- `cli/` is the only active implementation in the repository. An earlier
  Python `sprinkles` CLI and a top-level Go `dashboard-backend/` (built on
  Cloud Datastore and a GCE-"cluster"-based data model, distinct from
  sprinkles workpool/batch/worker Firestore model) have both been deleted from
  ...
  `cli/dev/dashboard_backend.go` directly (see
  [Deployment View](07-deployment-view.md)).
- The `monitor` package must not import the top-level `v100` package (and
  vice versa where `cli/dev` needs `monitor`) to avoid an import cycle —
