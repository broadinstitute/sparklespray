...
- `cli/build.sh [version]` cross-compiles a single static binary
  (`CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build ./cmd/sprinkles`) to
  `cli/bin/sprinkles-linux-amd64-<version>`, version-stamped via `-ldflags`.
...
- `cli/upload-worker-binary.sh [version] [gcs-path]` runs `build.sh` and
  then `gcloud storage cp`s the result to the fixed GCS path worker VMs
  bootstrap from (`sprinklesWorkerGCSPath` in `SprinklesConfig`/workpool
  specs — see §7.3), e.g. `gs://sprinkles-test-0625/bin/sprinkles-linux-amd64-dev`.
...
built and embedded directly into the `sprinkles` binary, so the whole control
...
- `cli/build.sh` runs `npm ci && npm run build` in `dashboard/`, copies the
  resulting `dashboard/dist/*` into `cli/dev/webui/dist/`, then
  cross-compiles `./cmd/sprinkles`. It skips the frontend build (an mtime
  ...
  overrides this. `cli/dev/webui/webui.go` `//go:embed all:dist`s the
...
- At runtime, `dashboard_backend.go`'s `http.ServeMux` registers
  ...
  browser.
- See [cli/docs/deploying-dashboard-backend.md](../deploying-dashboard-backend.md)
...
- Each Batch job's task group runs two runnables per VM:
  1. a throwaway `cloudsdktool/cloud-sdk:slim` container that
     `gcloud storage cp`s the `sprinkles` binary from
     `SprinklesWorkerGCSPath` onto the VM's `RootDir`;
  2. a script runnable that `chmod +x`s it and execs
     `sprinkles worker ...` **directly on the VM host** (not inside a
     container).
- The `sprinkles worker` process then shells out to the host's
...
- **`sprinkles serve`** is the deployment entry point: one long-lived process
  ...
  `cli/` prescribes a hosting platform; a small always-on VM or any
  container platform works.
- The halves can still be run separately — **`sprinkles dev monitor`** and
  **`sprinkles dev dashboard-backend`** — which is useful when restarting or
...
- `start.sh` shows the local-dev pattern: `mprocs` running `sprinkles serve`
...
| Cloud Pub/Sub           | `sprinkles-events`, `sprinkles-worker-in-<worker_id>`, `batch-api-notifications`    |
...
- Firestore, Pub/Sub, and GCS emulators (`gcloud beta emulators ...`,
  ...
  (`cli/functest/`, `cli/monitor/functest/`).
- `monitor/emulator/server.go` (launched via `sprinkles dev batchapi-emulator`) stands in for the real GCP Batch API, paired with
  `monitor/remote_batch_client.go` as its client.
- `sprinkles dev simulate` drives the monitor with a synthetic load
