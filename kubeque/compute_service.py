...
class ComputeService:
    ...
    def stop(self, name : str, zone : str) -> None :
        self.compute.instances().delete(project=self.project, zone=zone, instance=name).execute()

    def get_instance_status(self, zone : str, instance_name : str) -> str :
        try:
            instance = self.compute.instances().get(project=self.project, zone=zone, instance=instance_name).execute()
